-- =====================================================
-- SUPABASE SECURITY CONFIGURATION (FINAL CORRECTED)
-- Fix for RLS and security issues - Correct column names
-- =====================================================
...
CREATE POLICY "Users can view own wallets" ON public.user_wallets
    FOR SELECT USING (
        EXISTS (
            ...
            WHERE users.id = user_wallets."userId" 
...
CREATE POLICY "Users can view own TMA consents" ON public.tma_consents
    FOR SELECT USING (
        EXISTS (
            SELECT 1 FROM public.users 
            WHERE users.id = tma_consents."userId" 
            AND users."supabaseId" = auth.uid()::text
        )
    );
...
CREATE POLICY "Users can view own sellbacks" ON public.sellbacks
    FOR SELECT USING (
        EXISTS (
            SELECT 1 FROM public.users 
            WHERE users.id = sellbacks."sellerId" 
            AND users."supabaseId" = auth.uid()::text
        )
    );
...
CREATE POLICY "Users can view own reward claims" ON public.reward_claims
    FOR SELECT USING (
        EXISTS (
            SELECT 1 FROM public.users 
            WHERE users.id = reward_claims."userId" 
            AND users."supabaseId" = auth.uid()::text
        )
    );
...
CREATE POLICY "Users can view own rise conversions" ON public.rise_conversions
    FOR SELECT USING (
        EXISTS (
            SELECT 1 FROM public.users 
            WHERE users.id = rise_conversions."userId" 
            AND users."supabaseId" = auth.uid()::text
        )
    );
...
CREATE POLICY "Users can view relevant airdrop claims" ON public.airdrop_claims
    FOR SELECT USING (
        EXISTS (
            ...
            JOIN public.users ON users.id = wallets."userId"
            WHERE wallets.address = airdrop_claims."walletAddress" 
            AND users."supabaseId" = auth.uid()::text
        )
        OR 
        EXISTS (
            SELECT 1 FROM public.user_wallets 
            JOIN public.users ON users.id = user_wallets."userId"
            WHERE user_wallets.address = airdrop_claims."walletAddress" 
            AND users."supabaseId" = auth.uid()::text
        )
    );
...
-- Create a dedicated security schema and function (matches implemented version)
...
CREATE OR REPLACE FUNCTION security.user_owns_wallet(wallet_address text)
RETURNS boolean AS $$
BEGIN
    RETURN EXISTS (
        SELECT 1 FROM public.wallets 
...
-- Grant necessary permissions (restricted for security)
REVOKE ALL ON FUNCTION security.user_owns_wallet(text) FROM PUBLIC;
GRANT EXECUTE ON FUNCTION security.user_owns_wallet(text) TO authenticated;
