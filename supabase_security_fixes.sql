...
-- Fix for RLS and security issues
-- =====================================================
...
-- 3. Rewards and Staking Tables
ALTER TABLE public.epochs ENABLE ROW LEVEL SECURITY;
ALTER TABLE public.reward_claims ENABLE ROW LEVEL SECURITY;
ALTER TABLE public.staking_tracked_wallets ENABLE ROW LEVEL SECURITY;
ALTER TABLE public.staking_wallet_totals ENABLE ROW LEVEL SECURITY;
ALTER TABLE public.staking_claims ENABLE ROW LEVEL SECURITY;
...
-- Staking Tables - Users can only see their own
CREATE POLICY "Users can view own staking wallets" ON public.staking_tracked_wallets
    FOR SELECT USING (
        EXISTS (
            SELECT 1 FROM public.users 
            WHERE users.id = staking_tracked_wallets.user_id 
            AND users.supabase_id = auth.uid()::text
        )
    );

CREATE POLICY "Users can manage own staking wallets" ON public.staking_tracked_wallets
    FOR ALL USING (
        EXISTS (
            SELECT 1 FROM public.users 
            WHERE users.id = staking_tracked_wallets.user_id 
...
CREATE POLICY "Service role full access to staking wallets" ON public.staking_tracked_wallets
    FOR ALL USING (auth.role() = 'service_role');

-- Staking Totals
CREATE POLICY "Users can view own staking totals" ON public.staking_wallet_totals
    FOR SELECT USING (
        EXISTS (
            SELECT 1 FROM public.staking_tracked_wallets 
            JOIN public.users ON users.id = staking_tracked_wallets.user_id
            WHERE staking_tracked_wallets.id = staking_wallet_totals.tracked_wallet_id
            AND users.supabase_id = auth.uid()::text
        )
    );

CREATE POLICY "Service role full access to staking totals" ON public.staking_wallet_totals
    FOR ALL USING (auth.role() = 'service_role');

-- Staking Claims
CREATE POLICY "Users can view own staking claims" ON public.staking_claims
    FOR SELECT USING (
        EXISTS (
            SELECT 1 FROM public.users 
            WHERE users.id = staking_claims.user_id 
            AND users.supabase_id = auth.uid()::text
        )
    );

CREATE POLICY "Service role full access to staking claims" ON public.staking_claims
    FOR ALL USING (auth.role() = 'service_role');
